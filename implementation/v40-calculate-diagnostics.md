...
## 6.5 追補（2026-09-19）— 実装後に残った 2 つの課題を解いた

### 診断の大分類（課題: 列の指定ミスが 2 種別に分かれる）

種別は原因を細かく言い当てるが、**利用者（と LLM）が取る行動は種別より粗い**。`[列名]` で書いた打ち間違い（`unknown-column`）も裸で書いた打ち間違い（`unknown-name`）も、直し方は「列名を直す」で同じ。差し戻しのたびに消費側で束ね直すのは、束ね方が分かれる元になる。

`ExpressionDiagnostic` に `category` を足した（`empty` / `syntax` / `function` / `column` / `type` / `limit`）。種別 → 分類は `diagnosticCategory(code)` が持ち、診断を組み立てる口を 1 つ（`diagnostic()`）にしたので付け忘れが起きない。知らない種別は `syntax` に寄せる（投げない）。

`empty-column-name`（`[]`）も `column` に入れた。`unknown-name` を `column` にしたのは、裸の識別子の打ち間違いは関数名の可能性もあるが大半が列名で、直し方も列と同じため。

### 失敗の読み替え（課題: 型の警告と全行 null の相関を読者が取る必要がある）

`type-coerced` の警告は「寄せられる**かもしれない**」に留まり、それだけでは上流に型変換が要るか決められない。**実際の行を見て初めて「この列は数値にならない」と言い切れる**。

`ExpressionPreview` に `diagnosis: ExpressionPreviewDiagnosis` を足し、参照列ごとに実データの内訳（数値になった / 空でないのに数値にならなかった / 空）を数えて判断する。

| 条件 | `nextStep` |
|---|---|
| 空でない値が 1 つも数値にならなかった列がある | 「列 X の値を数値にできません。上流に型変換（cast）を入れてください。」 |
| 参照列の見た行がすべて空 | 「列 X は見た行がすべて空です。上流の null 処理で既定値を入れるか、参照する列を見直してください。」 |
| 全行が失敗し、最多の理由が 0 除算 | 「見た行すべてで 0 で割っています。分母の列を見直すか、0 の行を上流の行フィルターで除いてください。」 |
| 全行が失敗し、最多の理由が定義域の外 | 「見た行すべてが関数の定義域の外です。式を見直してください。」 |
| 一部の行だけ失敗 | 入れない（式もデータも概ね正しく、断定できる次の一手が無い） |

**一部の行だけ失敗したときに案内を出さない**のが要点。そこで断定すると、正常な式に毎回「直せ」と言うことになる。
