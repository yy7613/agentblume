...
- **状態**: プレビュー（ローカル実行向け。**認証は共有トークンまで実装済み・ロール権限と監査は未実装**）
...
| **ツールを作る** | ETLノードをつないで、決定的で再実行可能なデータ処理を作る。LLMを使わないのでプレビューが速く、結果が毎回同じ。**テンプレート**（前期比・期間内の統計・2つのデータの比・相関など）を選んでデータソースと列を指定するだけでも作れるし、**設計アシスタント**に文章で指示してノードを増やしていくこともできる |
...
| **自動生成する** | 「やりたいこと」とデータソースから、ツール・スキル・エージェント・検証資産一式をAgent Factoryが生成し、疑似ユーザー検証で自動改善する。ツールは人と同じテンプレートから作るので、小さなローカルモデルでも高度な構成（前期比・結合・比率）を組める |
...
- 既定のlistenは `127.0.0.1:3030`。別マシンから触らせる場合は `AGENTCONTEXT_HOST=0.0.0.0`。
  このとき **`AGENTCONTEXT_AUTH_TOKENS` の設定が必須**で、未設定なら起動を拒否する
  （認証なしの単一ユーザーモードのまま公開すると、誰でも全データを読み書き・削除できるため）。
  設定方法は [.env.example](.env.example) の「認証」節、仕組みは [docs/08-security-auth.md](docs/08-security-auth.md#0-実装状況2026-07)。
  ロールによる権限分けと監査ログは未実装なので、トークンを配った相手は全員が同じ操作をできる。
