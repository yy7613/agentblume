...
export function registerSkillRoutes(app: FastifyInstance, deps: SkillRouteDeps): void {
  ...
  app.delete<{ Params: { internalId: string } }>('/skills/:internalId', async (request, reply) => { const query = parse(scopeQuerySchema, request.query); await deps.deleteSkill.execute(query, request.params.internalId); return reply.status(204).send(); });
