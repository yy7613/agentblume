...
export function registerEvaluationAssetRoutes(app: FastifyInstance, deps: EvaluationAssetRouteDeps): void {
  ...
  app.post('/evaluator-profiles', async (request, reply) => {
    ...
    return reply.status(201).send({ profile: serializeEvaluatorProfile(await deps.saveEvaluatorProfile.execute({ ...withResolvedOwner(request, body), scope: scopeOf(request), metrics: body.metrics.map((metric) => metric.kind === 'code' ? metric : { ...metric, rubric: { id: metric.rubric.id, version: version(metric.rubric.version) as SemVer } }) })) });
  });
  ...
  app.post('/judge-rubrics', async (request, reply) => { const body = parse(saveJudgeRubricBodySchema, request.body); return reply.status(201).send({ rubric: serializeJudgeRubric(await deps.saveJudgeRubric.execute({ ...withResolvedOwner(request, body), scope: scopeOf(request) })) }); });
