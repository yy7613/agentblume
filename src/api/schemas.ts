 ...
 * - 業務（仕訳・経費精算…）のスキーマは `<業務>-schemas.ts` に置く（ADR-0039）。共通の `tenantScopeSchema` /
 *   `scopeQuerySchema` はここから import する。
...
/**
 * テキスト添付（docs/23 §9.4 C2）。ブラウザで PDF のテキスト層から抜いた契約書の本文などを、画像とは別の一覧で受ける。
 * 本文はモデルへのメッセージに載せず、ツールの実行文脈にだけ渡す（12B 級の文脈を本文で埋めない）。
 * 1 件 300,000 文字（契約書 1 通の本文上限と同じ）、最大 2 件・合計 400,000 文字。
 */
const TEXT_ATTACHMENT_TOTAL_MAX_CHARS = 400_000;
const textAttachmentsSchema = z.array(z.object({
  name: z.string().min(1).max(200),
  text: z.string().min(1).max(300_000),
  pageCount: z.number().int().min(1).max(500).optional(),
})).max(2).refine((documents) => documents.reduce((total, document) => total + document.text.length, 0) <= TEXT_ATTACHMENT_TOTAL_MAX_CHARS, {
  message: `documents must be at most ${TEXT_ATTACHMENT_TOTAL_MAX_CHARS} characters in total; attach fewer documents or split the contract`,
});
...
export const runAgentBodySchema = z.union([z.object({
  ...
  documents: textAttachmentsSchema.optional(),
}), z.object({
  ...runBaseSchema,
  agent: z.object({
    internalId: z.string().min(1),
