...
export function registerSessionRoutes(app: FastifyInstance, deps: SessionRouteDeps): void {
  app.post('/agent-sessions', async (request, reply) => {
    ...
    const session = await deps.createAgentSession.execute({ scope: scopeOf(request), agentId: body.agent.internalId, ...(version === undefined ? {} : { version }) });
    return reply.status(201).send({ session });
  });
  app.get<{ Params: { sessionId: string } }>('/agent-sessions/:sessionId', async (request) => {
    parseWith(sessionScopeQuerySchema, request.query);
    return { session: await deps.queryAgentSession.get(scopeOf(request), request.params.sessionId) };
  });
  app.post<{ Params: { sessionId: string } }>('/agent-sessions/:sessionId/close', async (request) => {
    parseWith(closeAgentSessionBodySchema, request.body);
    return { session: await deps.queryAgentSession.close(scopeOf(request), request.params.sessionId) };
  });
  app.get<{ Params: { sessionId: string } }>('/agent-sessions/:sessionId/artifacts', async (request) => {
    parseWith(sessionScopeQuerySchema, request.query);
    return { artifacts: await deps.querySessionArtifacts.list(scopeOf(request), request.params.sessionId) };
  });
  app.get<{ Params: { sessionId: string; artifactId: string } }>('/agent-sessions/:sessionId/artifacts/:artifactId', async (request) => {
    ...
    return await deps.querySessionArtifacts.get(scopeOf(request), request.params.sessionId, request.params.artifactId, query.limit, query.offset, query.section);
  });
  app.delete<{ Params: { sessionId: string; artifactId: string } }>('/agent-sessions/:sessionId/artifacts/:artifactId', async (request, reply) => {
    parseWith(sessionScopeQuerySchema, request.query);
    await deps.querySessionArtifacts.delete(scopeOf(request), request.params.sessionId, request.params.artifactId);
