...
export function registerQualityGateRoutes(app: FastifyInstance, deps: QualityGateRouteDeps): void {
  ...
  app.delete<{ Params: { id: string } }>('/gate-policies/:id', async (request, reply) => { parse(scopeQuerySchema, request.query); await deps.deleteGatePolicy.execute(scopeOf(request), request.params.id); return reply.status(204).send(); });
