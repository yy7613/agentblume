...
describe('RunRecord transitions', () => {
  it('runningからsucceededへ遷移し入力を複製する', () => {
    ...
    const run = succeedRun(started, { tool: { internalId: 'tool', version: '1.0.0' }, response: 'done', trace, usage: { totalTokens: 3 }, completedAt: '2026-07-03T00:00:01.000Z' });
    expect(run).toMatchObject({ status: 'succeeded', response: 'done', tool: { version: '1.0.0' } });
