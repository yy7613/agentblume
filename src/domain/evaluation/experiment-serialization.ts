...
export interface SerializedExperiment extends Omit<Experiment, 'target' | 'dataset' | 'evaluatorProfile' | 'judgeSamples'> {
  /** 後付け項目。古い JSON には無く、読み込み時は 1 として扱う。 */
...
export type SerializedExperimentCaseResult = Omit<ExperimentCaseResult, 'judgeEvaluations'> & {
  readonly judgeEvaluations?: readonly { readonly scorer: 'llm-as-judge'; readonly metricId: string; readonly rubric: { readonly id: string; readonly version: string }; readonly required: boolean; readonly model: Experiment['snapshot']; readonly status: 'succeeded' | 'failed'; readonly score?: number; readonly reason?: string; readonly error?: { readonly code: string; readonly message: string }; readonly criteria?: readonly { readonly id: string; readonly score: number | null; readonly reason: string }[]; readonly samples?: number; readonly dispersion?: { readonly min: number; readonly max: number; readonly stddev: number }; readonly uncertain?: boolean; readonly usage?: { readonly promptTokens?: number; readonly completionTokens?: number; readonly totalTokens?: number }; readonly contract?: { readonly promptHash: string; readonly rubricId: string; readonly rubricVersion: string } }[];
};
...
const experimentSchema = z.object({
  ...
  repetitions: z.number(), judgeSamples: z.number().optional(), status: z.enum(EXPERIMENT_STATUSES), snapshot: snapshotSchema, progress: z.object({ completed: z.number(), total: z.number() }), createdAt: z.string(), startedAt: z.string().optional(), finishedAt: z.string().optional(), error: z.object({ code: z.string(), message: z.string() }).optional(),
});
...
// 基準別判定（P1〜P4）の任意項目。古いレコードには無いので全て optional。
const judgeDetailsSchema = {
  criteria: z.array(z.object({ id: z.string(), score: z.number().nullable(), reason: z.string() })).optional(), samples: z.number().optional(), dispersion: z.object({ min: z.number(), max: z.number(), stddev: z.number() }).optional(), uncertain: z.boolean().optional(), usage: usageSchema.optional(),
  contract: z.object({ promptHash: z.string(), rubricId: z.string(), rubricVersion: z.string() }).optional(),
};
const resultSchema = z.object({
  experimentId: z.string(), scope: scopeSchema, caseId: z.string(), caseKind: z.enum(['turn', 'scenario']), repetition: z.number(), status: z.enum(EXPERIMENT_CASE_STATUSES), runIds: z.array(z.string()), output: z.string().optional(), scores: z.array(scoreSchema), latencyMs: z.number(), usage: usageSchema, error: z.object({ code: z.string(), message: z.string(), retryable: z.boolean() }).optional(),
  judgeEvaluations: z.array(z.object({ scorer: z.literal('llm-as-judge'), metricId: z.string(), rubric: z.object({ id: z.string(), version: z.string() }), required: z.boolean(), model: snapshotSchema, status: z.enum(['succeeded', 'failed']), score: z.number().optional(), reason: z.string().optional(), error: z.object({ code: z.string(), message: z.string() }).optional(), ...judgeDetailsSchema })).optional(),
});
