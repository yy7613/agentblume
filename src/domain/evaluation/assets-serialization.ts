...
const datasetSchema = z.object({ metadata: serializedPublishableMetadataSchema, cases: z.array(caseSchema) });
...
export function serializeEvaluationDataset(dataset: EvaluationDataset): SerializedEvaluationDataset {
  return { metadata: serializePublishableMetadata(dataset.metadata), cases: dataset.cases.map(serializeEvaluationCase) };
}
...
export function deserializeEvaluationDataset(value: unknown): EvaluationDataset {
  ...
  return createEvaluationDataset({ metadata: deserializePublishableMetadata(parsed.data.metadata, (text) => SemVer.parse(text)), cases: parsed.data.cases.map((entry) => deserializeEvaluationCase(entry as SerializedEvaluationCase)) });
}

export function serializeEvaluatorProfile(profile: EvaluatorProfile): SerializedEvaluatorProfile {
  return { metadata: serializePublishableMetadata(profile.metadata), metrics: profile.metrics.map((metric) => metric.kind === 'code' ? { ...metric } : { ...metric, rubric: { id: metric.rubric.id, version: metric.rubric.version.toString() } }) };
}
...
export function deserializeEvaluatorProfile(value: unknown): EvaluatorProfile {
  ...
  return createEvaluatorProfile({ metadata: deserializePublishableMetadata(parsed.data.metadata, (text) => SemVer.parse(text)), metrics: parsed.data.metrics.map((metric) => metric.kind === 'code' ? metric : { ...metric, rubric: { id: metric.rubric.id, version: SemVer.parse(metric.rubric.version) } }) });
}
