...
const TABLE = `CREATE TABLE IF NOT EXISTS evaluation_datasets (tenant_id TEXT NOT NULL, workspace_id TEXT NOT NULL, internal_id TEXT NOT NULL, version TEXT NOT NULL, major INTEGER NOT NULL, minor INTEGER NOT NULL, patch INTEGER NOT NULL, definition_json TEXT NOT NULL, PRIMARY KEY (tenant_id, workspace_id, internal_id, version));`;
const fromJson = (value: unknown): EvaluationDataset => deserializeEvaluationDataset(JSON.parse(String(value)));

export class SqliteEvaluationDatasetRepository implements EvaluationDatasetRepository {
  ...
  async findLatest(scope: TenantScope, id: string): Promise<EvaluationDataset | null> { const row = this.db.prepare(`SELECT definition_json FROM evaluation_datasets WHERE tenant_id=? AND workspace_id=? AND internal_id=? ORDER BY major DESC,minor DESC,patch DESC LIMIT 1`).get(scope.tenantId, scope.workspaceId, id); return row === undefined ? null : fromJson(row['definition_json']); }
  async listVersions(scope: TenantScope, id: string): Promise<SemVer[]> { return this.db.prepare(`SELECT major,minor,patch FROM evaluation_datasets WHERE tenant_id=? AND workspace_id=? AND internal_id=? ORDER BY major,minor,patch`).all(scope.tenantId, scope.workspaceId, id).map((row) => SemVer.of(Number(row['major']), Number(row['minor']), Number(row['patch']))); }
  async list(scope: TenantScope): Promise<EvaluationDatasetSummary[]> { return this.db.prepare(`SELECT d.definition_json FROM evaluation_datasets d WHERE d.tenant_id=? AND d.workspace_id=? AND d.version=(SELECT x.version FROM evaluation_datasets x WHERE x.tenant_id=d.tenant_id AND x.workspace_id=d.workspace_id AND x.internal_id=d.internal_id ORDER BY x.major DESC,x.minor DESC,x.patch DESC LIMIT 1) ORDER BY d.internal_id`).all(scope.tenantId, scope.workspaceId).map((row) => { const dataset = fromJson(row['definition_json']); return { internalId: dataset.metadata.internalId, displayName: dataset.metadata.displayName, publishName: dataset.metadata.publishName, latestVersion: dataset.metadata.version, state: dataset.metadata.state, caseCount: dataset.cases.length }; }); }
}
