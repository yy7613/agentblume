...
const TABLE = `CREATE TABLE IF NOT EXISTS judge_rubrics (tenant_id TEXT NOT NULL, workspace_id TEXT NOT NULL, internal_id TEXT NOT NULL, version TEXT NOT NULL, major INTEGER NOT NULL, minor INTEGER NOT NULL, patch INTEGER NOT NULL, definition_json TEXT NOT NULL, PRIMARY KEY (tenant_id, workspace_id, internal_id, version));`;
const fromJson = (value: unknown): JudgeRubric => deserializeJudgeRubric(JSON.parse(String(value)));
export class SqliteJudgeRubricRepository implements JudgeRubricRepository {
  ...
  async findLatest(scope: TenantScope, id: string): Promise<JudgeRubric | null> { const row = this.db.prepare(`SELECT definition_json FROM judge_rubrics WHERE tenant_id=? AND workspace_id=? AND internal_id=? ORDER BY major DESC,minor DESC,patch DESC LIMIT 1`).get(scope.tenantId, scope.workspaceId, id); return row === undefined ? null : fromJson(row['definition_json']); }
  async listVersions(scope: TenantScope, id: string): Promise<SemVer[]> { return this.db.prepare(`SELECT major,minor,patch FROM judge_rubrics WHERE tenant_id=? AND workspace_id=? AND internal_id=? ORDER BY major,minor,patch`).all(scope.tenantId, scope.workspaceId, id).map((row) => SemVer.of(Number(row['major']), Number(row['minor']), Number(row['patch']))); }
  async list(scope: TenantScope): Promise<JudgeRubricSummary[]> { return this.db.prepare(`SELECT r.definition_json FROM judge_rubrics r WHERE r.tenant_id=? AND r.workspace_id=? AND r.version=(SELECT x.version FROM judge_rubrics x WHERE x.tenant_id=r.tenant_id AND x.workspace_id=r.workspace_id AND x.internal_id=r.internal_id ORDER BY x.major DESC,x.minor DESC,x.patch DESC LIMIT 1) ORDER BY r.internal_id`).all(scope.tenantId, scope.workspaceId).map((row) => { const rubric = fromJson(row['definition_json']); return { internalId: rubric.metadata.internalId, displayName: rubric.metadata.displayName, publishName: rubric.metadata.publishName, latestVersion: rubric.metadata.version, state: rubric.metadata.state, criterionCount: rubric.criteria.length }; }); }
}
