...
const TABLE = `CREATE TABLE IF NOT EXISTS personas (tenant_id TEXT NOT NULL, workspace_id TEXT NOT NULL, internal_id TEXT NOT NULL, version TEXT NOT NULL, major INTEGER NOT NULL, minor INTEGER NOT NULL, patch INTEGER NOT NULL, definition_json TEXT NOT NULL, PRIMARY KEY (tenant_id, workspace_id, internal_id, version));`;
...
export class SqlitePersonaRepository implements PersonaRepository {
  ...
  async findLatest(scope: TenantScope, id: string): Promise<Persona | null> { const row = this.db.prepare(`SELECT definition_json FROM personas WHERE tenant_id=? AND workspace_id=? AND internal_id=? ORDER BY major DESC, minor DESC, patch DESC LIMIT 1`).get(scope.tenantId, scope.workspaceId, id); return row === undefined ? null : fromJson(row['definition_json']); }
  async listVersions(scope: TenantScope, id: string): Promise<SemVer[]> { return this.db.prepare(`SELECT major,minor,patch FROM personas WHERE tenant_id=? AND workspace_id=? AND internal_id=? ORDER BY major,minor,patch`).all(scope.tenantId, scope.workspaceId, id).map((row) => SemVer.of(Number(row['major']), Number(row['minor']), Number(row['patch']))); }
  async list(scope: TenantScope): Promise<PersonaSummary[]> { return this.db.prepare(`SELECT p.definition_json FROM personas p WHERE p.tenant_id=? AND p.workspace_id=? AND p.version=(SELECT x.version FROM personas x WHERE x.tenant_id=p.tenant_id AND x.workspace_id=p.workspace_id AND x.internal_id=p.internal_id ORDER BY x.major DESC,x.minor DESC,x.patch DESC LIMIT 1) ORDER BY p.internal_id`).all(scope.tenantId, scope.workspaceId).map((row) => { const persona = fromJson(row['definition_json']); return { internalId: persona.metadata.internalId, displayName: persona.metadata.displayName, publishName: persona.metadata.publishName, latestVersion: persona.metadata.version, archetype: persona.archetype, state: persona.metadata.state }; }); }
}
