...
export class SqlitePersonaRepository implements PersonaRepository {
  ...
  constructor(path = ':memory:') {
    ...
    const columns = this.db.prepare(`PRAGMA table_info(personas)`).all();
    if (!columns.some((column) => String(column['name']) === 'deleted')) {
      this.db.exec(`ALTER TABLE personas ADD COLUMN deleted INTEGER NOT NULL DEFAULT 0`);
    }
  }
