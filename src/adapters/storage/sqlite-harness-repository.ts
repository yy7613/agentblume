...
export class SqliteAgentHarnessRepository implements AgentHarnessRepository {
  ...
  async findLatest(scope: TenantScope, internalId: string): Promise<AgentHarness | null> {
    const row = this.db.prepare(`SELECT definition_json FROM agent_harnesses WHERE tenant_id = ? AND workspace_id = ? AND internal_id = ? ORDER BY major DESC, minor DESC, patch DESC LIMIT 1`).get(scope.tenantId, scope.workspaceId, internalId);
    return row === undefined ? null : fromJson(row['definition_json']);
  }
  async listVersions(scope: TenantScope, internalId: string): Promise<SemVer[]> { return this.db.prepare(`SELECT major, minor, patch FROM agent_harnesses WHERE tenant_id = ? AND workspace_id = ? AND internal_id = ? ORDER BY major, minor, patch`).all(scope.tenantId, scope.workspaceId, internalId).map((row) => SemVer.of(Number(row['major']), Number(row['minor']), Number(row['patch']))); }
  async list(scope: TenantScope): Promise<HarnessSummary[]> {
    const rows = this.db.prepare(`SELECT a.definition_json FROM agent_harnesses a WHERE a.tenant_id = ? AND a.workspace_id = ? AND a.version = (SELECT b.version FROM agent_harnesses b WHERE b.tenant_id = a.tenant_id AND b.workspace_id = a.workspace_id AND b.internal_id = a.internal_id ORDER BY b.major DESC, b.minor DESC, b.patch DESC LIMIT 1) ORDER BY a.internal_id`).all(scope.tenantId, scope.workspaceId);
