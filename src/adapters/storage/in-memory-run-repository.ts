...
export class InMemoryRunRepository implements RunRepository {
  ...
  async listAllByStatus(status: RunStatus): Promise<RunRecord[]> {
    return [...this.records.values()].filter((record) => record.status === status).sort((a, b) => a.startedAt.localeCompare(b.startedAt)).map(deserializeRun);
  }

  async listScopes(): Promise<TenantScope[]> {
    const scopes = new Map<string, TenantScope>();
    for (const record of this.records.values()) scopes.set(`${record.scope.tenantId}\u0000${record.scope.workspaceId}`, { ...record.scope });
