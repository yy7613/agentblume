...
const TABLE = `CREATE TABLE IF NOT EXISTS scenarios (tenant_id TEXT NOT NULL, workspace_id TEXT NOT NULL, internal_id TEXT NOT NULL, version TEXT NOT NULL, major INTEGER NOT NULL, minor INTEGER NOT NULL, patch INTEGER NOT NULL, definition_json TEXT NOT NULL, PRIMARY KEY (tenant_id, workspace_id, internal_id, version));`;
...
export class SqliteScenarioRepository implements ScenarioRepository {
  ...
  async findLatest(scope: TenantScope, id: string): Promise<Scenario | null> { const row = this.db.prepare(`SELECT definition_json FROM scenarios WHERE tenant_id=? AND workspace_id=? AND internal_id=? ORDER BY major DESC, minor DESC, patch DESC LIMIT 1`).get(scope.tenantId, scope.workspaceId, id); return row === undefined ? null : fromJson(row['definition_json']); }
  async listVersions(scope: TenantScope, id: string): Promise<SemVer[]> { return this.db.prepare(`SELECT major,minor,patch FROM scenarios WHERE tenant_id=? AND workspace_id=? AND internal_id=? ORDER BY major,minor,patch`).all(scope.tenantId, scope.workspaceId, id).map((row) => SemVer.of(Number(row['major']), Number(row['minor']), Number(row['patch']))); }
  async list(scope: TenantScope): Promise<ScenarioSummary[]> { return this.db.prepare(`SELECT s.definition_json FROM scenarios s WHERE s.tenant_id=? AND s.workspace_id=? AND s.version=(SELECT x.version FROM scenarios x WHERE x.tenant_id=s.tenant_id AND x.workspace_id=s.workspace_id AND x.internal_id=s.internal_id ORDER BY x.major DESC,x.minor DESC,x.patch DESC LIMIT 1) ORDER BY s.internal_id`).all(scope.tenantId, scope.workspaceId).map((row) => { const scenario = fromJson(row['definition_json']); return { internalId: scenario.metadata.internalId, displayName: scenario.metadata.displayName, publishName: scenario.metadata.publishName, latestVersion: scenario.metadata.version, state: scenario.metadata.state }; }); }
}
