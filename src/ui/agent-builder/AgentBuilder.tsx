...
import type { AgentKindDto, AgentPreviewRunDto, AgentSummaryDto, SerializedAgentDto, SideEffectDto, SkillSummaryDto, StructuredOutputFieldDto, StructuredOutputTypeDto, ToolSummaryDto } from '../api/types';
...
export function AgentBuilder({ client }: { readonly client: ToolApiClient }) {
  ...
  const effectiveSideEffect = useMemo<SideEffectDto | 'unknown'>(() => {
    const rank: Record<SideEffectDto, number> = { 'read-only': 0, write: 1, 'external-action': 2 };
  ...
  async function save(): Promise<void> {
    ...
    try {
      const agent = await client.saveAgent({ scope, internalId, workingName, displayName, publishName, owner, kind, systemPrompt, skills: skillRefs, tools: refs, agents: subAgentRefs, ...(output !== undefined ? { output } : {}) });
