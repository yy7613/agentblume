  ...
  return <main className="workspace-page mcp-page">
    ...
    <section className="mcp-publication">
      ...
      <div className="two-column-workspace">
        <section className="workspace-card"><h2>{text('Tool catalog', 'ツール一覧')}</h2>{tools.length === 0 && <p className="empty-state">{text('No Tools are available for publication.', '公開候補のツールがありません。')}</p>}{tools.map((tool) => <label className="agent-tool-option" key={tool.internalId}><input type="checkbox" checked={selected.has(tool.internalId)} onChange={() => toggle(tool.internalId)} /><span><strong>{tool.displayName}</strong><code>{tool.publishName}@{tool.latestVersion}</code></span><small>{tool.state}</small></label>)}</section>
