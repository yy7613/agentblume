...
function SlotCard({ slot, badge, agents, text, onAssign, removable, onRemove, onLabelChange, onPurposeChange }: {
  ...
  return <div className="harness-slot">
    ...
    <input className="harness-slot-label-input" aria-label={`${text('Slot label', 'Slot名')} ${slot.id}`} title={slot.label} value={slot.label} onChange={(event) => onLabelChange(event.target.value)} />
    {/* 目的文はcard幅で切れるため、途中で切れた事が分かるよう省略記号を出し、全文はtitleで読めるようにする。 */}
    <input className="harness-slot-purpose-input" aria-label={`${text('Slot purpose', 'Slotの目的')} ${slot.id}`} title={slot.purpose} style={{ textOverflow: 'ellipsis' }} value={slot.purpose} onChange={(event) => onPurposeChange(event.target.value)} />
...
function AggregatorCard({ label, purpose, assignment, agents, text, onAssign }: {
  ...
  return <div className="harness-slot harness-slot--aggregator">
    ...
    <strong>{label}</strong>
    <small title={purpose} style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{purpose}</small>
...
function HarnessCanvasBody({ pattern, slots, agents, text, assign, badge, onRemoveSlot, onLabelChange, onPurposeChange, onAddParticipant, aggregation, aggregatorSlot, onAggregatorAssign }: {
  ...
  switch (pattern) {
    ...
    case 'agent-as-tools': {
      ...
      return <div className="harness-hub-layout"><div className="harness-main-row"><InputNode text={text} /><Connector symbol="→" />{card(coordinator, 0, true)}<Connector symbol="→" /><OutputNode text={text} /></div><div className="harness-spokes">{participants.map((slot, index) => <div className="harness-spoke" key={slot.id}><Connector symbol="┆" label={text('ask', 'ask委譲')} dashed vertical />{card(slot, index + 1)}</div>)}{addButton}</div></div>;
    }
    case 'handoff': {
...
export function HarnessBuilder({ client }: { readonly client: ToolApiClient }) {
  ...
  function resetEditorState(): void {
    ...
    setSavedVersion(undefined); setValidation(undefined); setRunMessage(''); setRun(undefined); setSaveNotice(undefined);
  ...
  async function removeHarness(target: string): Promise<void> {
    setBusy('load'); setError(undefined);
    try { await client.deleteHarness(target, scope); setPendingDelete(undefined); await refreshHarnesses(); }
  ...
  function populateEditorFromHarness(harness: SerializedAgentHarnessDto): void {
    ...
    setValidation(undefined); setRun(undefined); setRunMessage(''); setSaveNotice(undefined);
    setEditing(true);
  }
  ...
  const ready = internalId.trim() !== '' && displayName.trim() !== '' && slotStates.every((slot) => slot.assignment.internalId !== '') && aggregatorReady;
  // 保存ボタンが押せない理由。検証ボタンを押さなくても見えるよう、ボタン近傍へ常時表示する。
  const blockingReasons = useMemo(() => {
    ...
    if (missing.length > 0) reasons.push(text(`Required fields are empty: ${missing.join(separator)}.`, `${missing.join(separator)}が未入力です。`));
    const unassigned = resolvedSlots.filter((slot) => slot.assignment.internalId === '').map((slot) => slot.label);
    if (unassigned.length > 0) reasons.push(text(`Assign a saved Agent version to every slot: ${unassigned.join(separator)}.`, `保存済みAgentのversionが未割当のslotがあります: ${unassigned.join(separator)}。`));
    if (!aggregatorReady) reasons.push(text('Assign a saved Agent version to the Aggregator slot.', '集約役のslotへ保存済みAgentのversionを割り当ててください。'));
    return reasons;
  }, [internalId, displayName, resolvedSlots, aggregatorReady, text, language]);
  // 「previewで実行可能」は検証・保存が通った時だけ緑にする（未検証・未充足では中立/エラー表示）。
  const previewStatus = validation?.valid === true
    ? { className: 'validation-status good', label: text('Executable in preview', 'previewで実行可能') }
    : validation !== undefined ? { className: 'validation-status bad', label: text('Validation failed', '検証エラーあり') }
      : ready ? { className: 'validation-status', label: text('Not validated yet', '未検証') }
        : { className: 'validation-status', label: text('Definition incomplete', '定義が未完成') };
  ...
  if (view === 'list') {
    return <main className="harness-builder harness-list-page">
      ...
      <ConfirmDialog open={pendingDelete !== undefined} title={text('Delete multi-agent', 'マルチエージェントを削除')}
        ...
        onConfirm={() => { if (pendingDelete !== undefined) void removeHarness(pendingDelete.internalId); }} onCancel={() => setPendingDelete(undefined)} />
    </main>;
  }
  return <main className="harness-builder">
    ...
    <div className="harness-layout"><aside className="harness-presets"><h2>{text('Patterns', 'パターン')}</h2>{patterns.map((item) => <button key={item.id} type="button" className={pattern === item.id ? 'active' : ''} onClick={() => choosePattern(item.id)}><strong>{item.label}</strong><small>{text(item.noteEn, item.noteJa)}</small></button>)}</aside>
      <section className="harness-workspace"><div className="harness-fields"><label>{text('Internal ID', '内部ID')}<span className="required-mark">*</span><input aria-label={text('Internal ID', '内部ID')} value={internalId} readOnly={editing} title={editing ? text('Internal ID cannot change once saved (it would fork a new asset).', '保存済みの内部IDは変更できません（変更すると別資産になります）。') : undefined} onChange={(event) => { if (editing) return; setInternalId(event.target.value); setSavedVersion(undefined); }} placeholder="content-review" /></label><label>{text('Display name', '表示名')}<span className="required-mark">*</span><input aria-label={text('Display name', '表示名')} value={displayName} onChange={(event) => { setDisplayName(event.target.value); setSavedVersion(undefined); }} placeholder="Content Review" /></label><label>{text('Owner', '所有者')}<input aria-label={text('Owner', '所有者')} value={owner} onChange={(event) => { setOwner(event.target.value); setSavedVersion(undefined); }} placeholder={text('Optional (defaults to you)', '省略可（空欄なら自分の名前）')} /></label></div>
        <div className={`harness-canvas harness-canvas--${pattern}`} aria-label={text('Multi-agent canvas', 'マルチエージェントキャンバス')}><HarnessCanvasBody pattern={pattern} slots={resolvedSlots} agents={agents} text={text} assign={assign} badge={badge} onRemoveSlot={removeSlot} onLabelChange={editSlotLabel} onPurposeChange={editSlotPurpose} onAddParticipant={addParticipant} aggregation={aggregation} aggregatorSlot={aggregatorSlot} onAggregatorAssign={assignAggregator} /></div>
        <div className={`harness-inspector${pattern === 'concurrent' ? ' harness-inspector--concurrent' : ''}`}><div><span className="eyebrow">{text('Pattern inspector', 'パターン設定')}</span><h2>{selected?.label}</h2><p>{selected !== undefined ? text(selected.noteEn, selected.noteJa) : ''}</p>{pattern === 'agent-as-tools' && <p className="empty-state">{text('Tip: a single agent delegating to others can also be built with sub-agents on the Agent screen. Use this pattern when you want version-pinned slots, budgets, and run events without editing the agent.', 'ヒント: 単一エージェントから他エージェントへ委譲するだけなら、エージェント画面の「サブエージェント」でも同等のことができます。エージェント定義を変えずに版固定・予算・実行イベントを付けたい場合にこのパターンを使います。')}</p>}</div><label>{text('Failure policy', '失敗時の方針')}<select value={policy.failure.mode} disabled><option>fail-fast</option></select></label><label>{text('Max parallelism', '最大並列数')}<input value={policy.budget.maxParallelism} disabled /></label>{pattern === 'concurrent' && <label>{text('Aggregation', '集約方法')}<select value={aggregation} onChange={(event) => chooseAggregation(event.target.value as AggregationMode)}><option value="collect">{text('Concatenate mechanically (collect)', '機械的に連結 (collect)')}</option><option value="vote">{text('Majority vote (vote)', '投票で決定 (vote)')}</option><option value="agent">{text('Synthesize with an Agent (agent)', 'Agentで統合 (agent)')}</option></select></label>}<p className={previewStatus.className}>{previewStatus.label}</p><div className="harness-topology-summary">{topologySummaryRows(currentTopology, text, aggregatorAssignment).map((row) => <span key={row.key} className="harness-topology-row">{row.label}: {row.value}</span>)}</div></div>
        ...
        <div className="harness-preview"><div><span className="eyebrow">{text('Saved multi-agent preview', '保存済みマルチエージェントのプレビュー')}</span><h2>{savedVersion === undefined ? text('Save to run', '保存後に実行') : `${internalId}@${savedVersion}`}</h2></div><textarea aria-label={text('Multi-agent chat message', 'マルチエージェントへのメッセージ')} value={runMessage} onChange={(event) => setRunMessage(event.target.value)} placeholder={text('Ask the multi-agent…', 'マルチエージェントへ依頼…')} /><button type="button" className="primary" disabled={savedVersion === undefined || runMessage.trim() === '' || busy !== undefined} onClick={() => void preview()}>{busy === 'run' ? text('Running…', '実行中…') : text('Run preview', 'previewを実行')}</button>{run !== undefined && <div className={`harness-run ${run.status}`}><strong>{run.status}</strong><p>{run.response ?? run.failure?.message}</p><small>{run.events.filter((event) => event.kind === 'participant_completed').length} {text('participant runs', '参加Agent実行')}</small></div>}</div>
      </section></div>
  </main>;
}
