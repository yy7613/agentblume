...
describe('structured node configuration dialogs', () => {
  it('opens structured editors for complex nodes and keeps Cancel changes out of the graph', async () => {
    ...
    const cases: Array<{ type: 'agent-input' | 'json-source' | 'csv-source' | 'rename' | 'cast' | 'join' | 'sort' | 'fill-null' | 'replace' | 'workspace-output'; add?: string }> = [
      { type: 'agent-input', add: 'Add column' }, { type: 'json-source' }, { type: 'csv-source' },
