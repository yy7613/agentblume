...
type Text = ReturnType<typeof useI18n>['text'];
function archetypeLabel(text: Text, value: PersonaArchetypeDto): string {
  return { novice: text('novice', '初心者'), expert: text('expert', '上級者'), busy: text('busy', '多忙'), vague: text('vague', '曖昧'), skeptical: text('skeptical', '懐疑的'), custom: text('custom', 'カスタム') }[value];
}
function levelLabel(text: Text, value: PersonaLevelDto): string {
  return { low: text('low', '低'), mid: text('mid', '中'), high: text('high', '高') }[value];
}
function verbosityLabel(text: Text, value: PersonaVerbosityDto): string {
  return { terse: text('terse', '簡潔'), normal: text('normal', '普通'), chatty: text('chatty', '饒舌') }[value];
}
function languageLabel(text: Text, value: PersonaLanguageDto): string {
  return { ja: text('Japanese', '日本語'), en: text('English', '英語') }[value];
}
  ...
  return <>
      ...
      <div className="validation-list">
        ...
        {personas.map((persona) => <div className="validation-list-row" key={persona.internalId}>
          <button type="button" className={persona.internalId === editingId ? 'selected' : ''} onClick={() => void edit(persona)}>
            <span className="archetype-badge">{archetypeLabel(text, persona.archetype)}</span><strong>{persona.displayName}</strong><span className="version-chip">{persona.latestVersion}</span>
          </button>
          <button type="button" className="secondary danger" disabled={busy} onClick={() => setPendingDelete({ id: persona.internalId, name: persona.displayName })}>{text('Delete', '削除')}</button>
        </div>)}
      </div>
    </section>
    <section className="workspace-card" aria-label={text('Persona form', 'ペルソナフォーム')}>
      <div className="panel-title"><h2>{editingId === undefined ? text('New persona', '新規ペルソナ') : text('Edit persona', 'ペルソナを編集')}</h2><div className="save-actions">
        ...
        <select aria-label={text('Persona version bump', 'ペルソナのバージョン更新種別')} value={bump} onChange={(event) => setBump(event.target.value as typeof bump)}><option value="patch">{text('patch', 'パッチ')}</option><option value="minor">{text('minor', 'マイナー')}</option><option value="major">{text('major', 'メジャー')}</option></select>
      ...
      <div className="agent-fields">
        ...
        <label>{text('Verbosity', '発話量')}<select aria-label={text('Persona verbosity', 'ペルソナ発話量')} value={verbosity} onChange={(event) => setVerbosity(event.target.value as PersonaVerbosityDto)}>{VERBOSITIES.map((item) => <option key={item} value={item}>{verbosityLabel(text, item)}</option>)}</select></label>
        <label>{text('Language', '言語')}<select aria-label={text('Persona language', 'ペルソナ言語')} value={language} onChange={(event) => setLanguage(event.target.value as PersonaLanguageDto)}>{LANGUAGES.map((item) => <option key={item} value={item}>{languageLabel(text, item)}</option>)}</select></label>
      </div>
    ...
    <ConfirmDialog
      ...
      title={text('Delete persona', 'ペルソナを削除')}
