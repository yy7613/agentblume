  ...
  return <>
    ...
    <div className="two-column-workspace">
      ...
      <div className="evaluation-editors">
        ...
        <section className="workspace-card" aria-label="Judge rubric editor">
          <div className="panel-title"><h2>Judge rubric</h2><div className="save-actions">{rubricSavedVersion !== undefined && <span className="version-chip">saved {rubricSavedVersion}</span>}<select aria-label="Judge rubric version bump" value={rubricBump} onChange={(event) => setRubricBump(event.target.value as typeof rubricBump)}><option>patch</option><option>minor</option><option>major</option></select><button type="button" className="primary" disabled={busy !== undefined || !rubricValid} onClick={() => void saveRubric()}>{busy === 'rubric' ? 'Saving…' : 'Save rubric'}</button></div></div>
          <div className="agent-fields"><label>Internal ID<input aria-label="Judge rubric internal ID" value={rubricId} onChange={(event) => setRubricId(event.target.value)} /></label><label>Working name<input aria-label="Judge rubric working name" value={rubricWorkingName} onChange={(event) => setRubricWorkingName(event.target.value)} /></label><label>Display name<input aria-label="Judge rubric display name" value={rubricDisplayName} onChange={(event) => setRubricDisplayName(event.target.value)} /></label><label>Publish name<input aria-label="Judge rubric publish name" value={rubricPublishName} onChange={(event) => setRubricPublishName(event.target.value)} /></label><label>Owner<input aria-label="Judge rubric owner" value={rubricOwner} onChange={(event) => setRubricOwner(event.target.value)} /></label><label>Reference policy<select aria-label="Judge reference policy" value={referencePolicy} onChange={(event) => setReferencePolicy(event.target.value as typeof referencePolicy)}><option value="optional">optional</option><option value="required">required</option><option value="forbidden">forbidden</option></select></label></div>
          <label>Instructions<textarea aria-label="Judge rubric instructions" rows={3} value={rubricInstructions} onChange={(event) => setRubricInstructions(event.target.value)} /></label>
          <div className="evaluation-cases">{criteria.map((criterion, index) => <div className="evaluation-case" key={index}><div className="evaluation-case-head"><input aria-label={`Criterion ${index + 1} ID`} value={criterion.id} onChange={(event) => setCriteria((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, id: event.target.value } : item))} /><input aria-label={`Criterion ${index + 1} label`} value={criterion.label} onChange={(event) => setCriteria((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, label: event.target.value } : item))} /><input aria-label={`Criterion ${index + 1} weight`} type="number" min="0.1" step="0.1" value={criterion.weight} onChange={(event) => setCriteria((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, weight: Number(event.target.value) } : item))} /><button type="button" className="secondary" aria-label={`Remove criterion ${index + 1}`} disabled={criteria.length === 1} onClick={() => setCriteria((items) => items.filter((_, itemIndex) => itemIndex !== index))}>×</button></div><label>Description<textarea aria-label={`Criterion ${index + 1} description`} value={criterion.description} onChange={(event) => setCriteria((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, description: event.target.value } : item))} /></label>{criterion.levels.map((level, levelIndex) => <div className="metric-row" key={levelIndex}><input aria-label={`Criterion ${index + 1} level ${levelIndex + 1} score`} type="number" min="0" max="1" step="0.1" value={level.score} onChange={(event) => setCriteria((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, levels: item.levels.map((entry, entryIndex) => entryIndex === levelIndex ? { ...entry, score: Number(event.target.value) } : entry) } : item))} /><input aria-label={`Criterion ${index + 1} level ${levelIndex + 1} label`} value={level.label} onChange={(event) => setCriteria((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, levels: item.levels.map((entry, entryIndex) => entryIndex === levelIndex ? { ...entry, label: event.target.value } : entry) } : item))} /><input aria-label={`Criterion ${index + 1} level ${levelIndex + 1} description`} value={level.description} onChange={(event) => setCriteria((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, levels: item.levels.map((entry, entryIndex) => entryIndex === levelIndex ? { ...entry, description: event.target.value } : entry) } : item))} /></div>)}</div>)}</div>
          <button type="button" className="secondary" onClick={() => setCriteria((items) => [...items, newCriterion(items.length + 1)])}>Add criterion</button>
        </section>
      </div>
    </div>
  </>;
