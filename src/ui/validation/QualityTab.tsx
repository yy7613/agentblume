...
import { caseResultStatusLabel, gateStatusLabel } from './status-labels';
...
/**
 * 基準別の派生指標は `<metricId>:<criterionId>` の名前で届く（例 `judge:accuracy`）。
 * 合成指標の行の直後にその基準行を並べ、「metric › criterion」で字下げして 1 つの群として読めるようにする。
 * 親の無い基準行（合成指標が比較不能で落ちた等）は末尾へ、素の指標は元の順のまま。
 */
export function splitMetricName(metric: string): { readonly parent: string; readonly criterion?: string } {
  const at = metric.indexOf(':');
  if (at <= 0 || at === metric.length - 1) return { parent: metric };
  return { parent: metric.slice(0, at), criterion: metric.slice(at + 1) };
}
export function groupMetricRows<T extends { readonly metric: string }>(rows: readonly T[]): readonly T[] {
  const children = rows.filter((row) => splitMetricName(row.metric).criterion !== undefined);
  const placed = new Set<T>();
  const grouped: T[] = [];
  for (const row of rows) {
    if (splitMetricName(row.metric).criterion !== undefined) continue;
    grouped.push(row);
    for (const child of children) { if (splitMetricName(child.metric).parent === row.metric) { grouped.push(child); placed.add(child); } }
  }
  return [...grouped, ...children.filter((child) => !placed.has(child))];
}
...
export function QualityTab({ client, scope }: { readonly client: ToolApiClient; readonly scope: TenantScopeDto }) {
  ...
  const [regressionsOnly, setRegressionsOnly] = useState(false); const [policyOwner, setPolicyOwner] = useState('reviewer');
  ...
  async function savePolicy(): Promise<void> {
    await run(async () => {
      ...
      const policy = await client.saveGatePolicy({ scope, internalId: newPolicyId, workingName: newPolicyId, displayName: newPolicyId, publishName: newPolicyId.replaceAll('-', '_'), owner: policyOwner, rules: [{ id: 'threshold', kind: 'metric-threshold', metric, operator, threshold }, { id: 'regression', kind: 'max-regression', metric, maxRegression }, ...(tags.length > 0 ? [{ id: 'required-tags', kind: 'required-case-pass' as const, tags }] : [])] });
  ...
  async function promote(): Promise<void> { const candidate = experiments.find((item) => item.id === candidateId); if (candidate === undefined || report === undefined) return; await run(async () => { await client.requestPromotion(candidate.target.agentId, candidate.target.version, scope, report.id); await refresh(); }); }
  async function decide(request: PromotionRequestDto, decision: 'approve' | 'reject'): Promise<void> { await run(async () => { await client.decidePromotion(request.id, decision, scope); await refresh(); }); }
  ...
  const metricRows = useMemo(() => comparison === undefined ? [] : groupMetricRows(comparison.metrics), [comparison]);
  // ゲート指標の候補: 比較で見えた指標（基準別も含む）と既定の成功率。入力は自由記述のまま（比較前でも保存できる）。
  const metricOptions = useMemo(() => Array.from(new Set(['case-success-rate', ...(comparison?.metrics.map((item) => item.metric) ?? [])])), [comparison]);
