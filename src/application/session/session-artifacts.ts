...
export class QuerySessionArtifactsUseCase {
  ...
  async list(scope: TenantScope, sessionId: string): Promise<readonly SessionArtifactDescriptor[]> {
  ...
  async get(scope: TenantScope, sessionId: string, artifactId: string, limit = 100, offset = 0, section?: 'nodes' | 'edges'): Promise<{ readonly artifact: SessionArtifactDescriptor; readonly payload: unknown }> {
    await this.sessions.get(scope, sessionId);
...
export function assertSessionMatchesAgent(session: AgentSession, agent: { readonly internalId: string; readonly version: string }): void {
