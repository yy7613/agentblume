...
describe('GetModelSettingsUseCase', () => {
  it('未保存ならスコープだけを返す（= env 既定を使う）', async () => {
    const { get } = make();
    expect(await get.execute(scope)).toEqual({ scope });
  });
...
describe('SaveModelSettingsUseCase', () => {
  ...
  it('source を切り替えても既存キーは維持される（フォームでキーを再入力させない）', async () => {
    const { save } = make();
    ...
    expect(view.main).toEqual({ source: 'openai-compatible', baseUrl: 'https://api.example.com/v1', model: 'gpt-4o', apiKey: { configured: true, hint: '4321' } });
  });
