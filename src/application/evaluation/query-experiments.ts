...
export class QueryExperimentsUseCase {
  ...
  async results(scope: TenantScope, experimentId: string): Promise<ExperimentCaseResult[]> { await this.get(scope, experimentId); return this.repo.listCaseResults(scope, experimentId); }
  async get(scope: TenantScope, id: string): Promise<Experiment> { const value = await this.repo.find(scope, id); if (value === null) throw new ExperimentNotFoundError(`Experiment not found: ${id}`); return value; }
}
