...
export class QueryEvaluationDatasetsUseCase {
  ...
  versions(scope: TenantScope, internalId: DatasetId): Promise<SemVer[]> { return this.repo.listVersions(scope, internalId); }
  async get(scope: TenantScope, internalId: DatasetId, version?: SemVer): Promise<EvaluationDataset> {
...
export class DeleteEvaluationDatasetUseCase {
  constructor(private readonly repo: EvaluationDatasetRepository) {}
  async execute(scope: TenantScope, internalId: DatasetId): Promise<void> {
