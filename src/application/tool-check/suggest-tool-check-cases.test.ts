...
import { TOOL_CHECK_CASES_PROMPT, SuggestToolCheckCasesUseCase, type SuggestToolCheckCasesInput } from './suggest-tool-check-cases';
...
async function harness(options: HarnessOptions = {}) {
  ...
  const useCase = new SuggestToolCheckCasesUseCase(repo, new EtlEngine(createDefaultRegistry()), options.model ?? scripted, () => options.enabled ?? true, bundledPrompts(), options.resolveDataSources, options.modelSnapshot, options.resolveAiJudgments);
...
/**
 * v48 でこの文を `prompts/tool-check/suggest-cases.md` へ移した。移行は**等価変換**なので、
 * モデルへ渡る system が移行前と一字一句同じであることをここで固定する
 * （`toContain` では、カテゴリの定義が 1 行消えても気づけない）。
 */
describe('検証ケース提案のプロンプト: 文をファイルへ移しても組み立てた文は変わらない', () => {
  it('従来どおり: system は 11 行（形式・引数規則・3 カテゴリ・期待の規則・信頼境界）のまま', async () => {
    const { scripted } = await suggest(fullResponse());
    expect(scripted.requests[0]?.messages[0]?.content).toBe([
  ...
  it('従来どおり: 版はファイルの frontmatter が正（コードに定数を持たない）', () => {
    expect(bundledPrompts().get(TOOL_CHECK_CASES_PROMPT.id).version).toBe('suggest-cases/v1');
  });
});
