...
- サブエージェント間の**直接対話**（サブ同士のP2P会話）。ルート経由のツリー委譲のみ
- 並列委譲（同時に複数サブを走らせる）。v17は逐次
- 動的なエージェント選択（レジストリからLLMが任意のAgentを発見して呼ぶ）。参照は保存時に固定
- 会話状態のサブエージェント間共有（各委譲は独立した`message`で完結。継続文脈が要るならルートが要約して渡す）
