...
> **仕訳（`journal-entries`）ノードは、この画面のノードパレットには出ない。** ドメイン層にはノード実装があり（status（draft / confirmed / exported）、期間（from / to）、件数上限を設定でき、固定14列・1行=1仕訳行を返す）、組み込みツール `builtin-journal-entries` がサーバー側でこのノードを使ったグラフを組んで公開する（[20-journal.md §14](./20-journal.md)）。利用者が Tool Builder の「ノード」パネルから手で置ける入力ノードではない（`src/ui/tool-builder/node-catalog.ts` の `NODE_CATALOG` に含まれない）。
...
基本統計、相関、時系列、外れ値の設定Dialogでは、schemaと決定的data profileから推奨初期値を作れる。`LM_STUDIO_MODEL`が設定され、structured outputを利用できる場合だけ、見出し「**ローカルLLM設定補助**」の区画とボタン「**設定案を作成**」を表示する。
...
> **日付の範囲指定に渡す値の形**: `filter` の大小比較（`gt` / `gte` / `lt` / `lte`）を `periodStart`（`date` 列）にかけるとき、`value` は **ISO 日付文字列**（`'2008-01-01'`。日時も可）でよい。列が日付なら、比較する前に自動でその文字列を `Date` へ変換する（保存済み Tool 定義は JSON のため元々 `Date` リテラルは持てず、Agent Tool の引数も文字列で届く。§3.8「日付列の条件値は ISO 文字列で書く」と同じ規則）。**ISO として読めない文字列**（`'2008/01/01'`・`'昨日'` など）は**エラー**にする（設計時は `filter: value for date column ... must be an ISO date (YYYY-MM-DD): ...` の error issue、実行時は同じ文言の `ETL_SCHEMA` 例外）。以前はここが NaN 同士の比較になり、エラーも出さずに 0 行を返していたが、いまは自動変換とエラー化で直っている（[§3.8](#38-agent-input-の条件バインドと公開契約)参照）。
...
4. **エージェントの引数を決める** — スロットを埋めると「**エージェントの引数**」の区画に、そのツールが受け取る引数が1行ずつ並ぶ（引数名・説明・「**必須**」チェック。初期値はテンプレートの既定）。「必須」を外した引数は、**エージェントが省略するとその条件で絞らない**（束縛された条件ごと飛ばされる）。テンプレート作者が `lock` を書いた引数は切り替えられず、理由がその行に出る。同梱テンプレートでは粒度（`granularity`）を固定している（任意にすると、省略されたときに月次と年次が混ざるため）。作成後でも「エージェント入力」ノードの「任意」で変えられる。
5. **名前を決める** — 「**ツール名**」（表示名。日本語可）と「**関数名**」（モデルに見える名前。英数字・`_`・`-` で1〜64文字）を**必ず入力する**。テンプレート側の名前を既定にしない理由は、同じテンプレートから作った2本目が1本目と同じ内部ID・公開名になり、**別のToolのつもりで保存すると既存Toolの新しいバージョンになってしまう**ため。保存済みToolと同じ関数名はその場で弾く（保存まで待たない）。関数名は内部ID・公開名・関数名の3つに、ツール名は表示名・作業名に入る。
6. **作成** — サーバーが実体化し、**手で組んだ Tool と同じ検査**（構造・列の存在・設計時プレビュー）を通してからグラフを返す。画面はそれをキャンバスへ展開し、表示名・公開名・Agent Tool 契約（名前と説明）を埋める。保存はしない（内容を確かめてから明示的に保存する。所有者は省略でき、空欄ならログイン中の利用者名が入る）。
7. 展開後、どのテンプレートから作ったか（`id@version`）を通知として出す。
...
### 3.17 設計アシスタント（文章で指示するとノードが増える）

ツール作成画面のヘッダ（メタデータの並び）に「**設計アシスタント**」ボタンがある。押すと、Claude Code や Codex のように**自由な文章で指示するといまのキャンバスにノードが増えたり設定が変わったりする**チャットパネルが右側（`NodeInspector` の隣、幅 320px）に開く。もう一度押すか × で閉じる。開閉はブラウザに覚えさせる。

「都道府県別の人口を年次に絞って多い順に10件返して」のように、まだ何も置いていない空のキャンバスからでも書き始められる。**保存はしない** — 返るのはキャンバスへ展開する編集後のグラフで、保存は従来どおり「バージョンを保存」。

**流れ**

1. パネル下部の入力欄（複数行）に指示を書き、Enter で送信（Shift+Enter は改行。日本語入力の変換確定中の Enter は送らない）。送信中は入力欄を無効にし「考えています…」と出す。
2. サーバーが、いまのグラフ・上流の列（スキーマ）・終端の標本行・登録済みデータソースの一覧とプロファイルを材料に、モデルへ「いまのグラフに対する**編集操作**」を書かせる（グラフ全体は書かせない）。
3. 返答に編集後のグラフが含まれていれば、**確認ダイアログなしに即座にキャンバスへ適用**する。取り消しは各ターンの下に出る「この変更を取り消す」で行う（そのターン以降の変更もまとめて戻り、会話は残ったまま応答に「取り消し済み」と付く）。
4. 新しく増えたノードは、挿入元ノードの右（+220px）に置く。挿入元が無ければ既存の最右列のさらに右に縦に並ぶ。追加・変更したノードは4秒間ハイライトし、最初に変更したノードを選択状態にする。
5. 返答には変更の一覧が並ぶ。サーバーは英語の定型文（`added filter 'filter-region' after 'src-sales'` など。モデルへ戻す会話にもこのまま使う）で返し、画面は表示のときだけ日本語にする（ノードの種別はパレットと同じ名前。`src/ui/api/error-messages.ts` の `localizeDesignChatChange`）。質問への回答や聞き返しなど、キャンバスを変えない返答は変更一覧が空のまま返る。**返答の文ではなく変更一覧が正**で、適用できなかったターンは赤枠の先頭に「適用していません（キャンバスは変わっていません）」と出る。プロンプトの規則（`design-chat/v3`）は「返答文は返した操作だけを述べる」「頼まれていないキー・列・ノードを足さない（足すなら理由を言う）」をモデルへ指示している。
6. 直近12件の発言（ユーザー・アシスタントそれぞれの1メッセージを1件と数える。往復では最大6ターン分。`DESIGN_CHAT_TRANSCRIPT_LIMIT` / `DESIGN_CHAT_MAX_TURNS`）までの会話を次の指示と一緒に送るので、「続けて地域を引数で絞れるように」のように前のターンを踏まえた指示が書ける。別のツールを開く・新規作成・テンプレートから作成すると会話は消える。

**操作の語彙（モデルが書けるのはこの6つだけ）**

| 操作 | 意味 |
|---|---|
| `add-node` | ノードを足す。挿入元を指定すると、その直後の鎖へ割り込む（挿入元の出力エッジを新ノードへ付け替える。出力が2本以上の分岐点では、どの枝の手前かを黙って決めずに枝を1本増やすだけにする） |
| `remove-node` | ノードを外す。入力1・出力1の鎖の途中なら上流と下流を直結する。それ以外は繋がずに外す |
| `set-config` | 設定を丸ごと置き換える |
| `connect` / `disconnect` | ノード間の接続を足す・外す |
| `set-agent-tool` | グラフの操作ではなく、Tool Calling 契約（エージェント向けの説明文・関数名）を書き換える。詳細は下の「Tool Calling 契約の説明の更新」 |

グラフへの当てはめは決定的な純関数（配線をモデルに書かせない）で行い、違反（存在しないノードID・重複するIDなど）は理由と直し方つきで弾く。当てた後は既存の正規化・スキーマ伝播・設計時プレビューへ通し、落ちたら理由を添えて**1回だけ**モデルに出し直させる（関数電卓の式提案・[§3.14](#314-関数電卓ノード数式で計算列を足す)と同じ「提案 → 検分 → 1回だけ差し戻し」）。差し戻しても通らなければ**キャンバスは変えず**、説明と理由だけを返す（理由は返答の下に赤い枠で出る）。差し戻しが起きたかどうかは応答に載るが、画面には出さない（成功した応答に赤枠は要らないため）。

**検証を通っても答えが間違う形（意味の検査）**: 実測で 12B が繰り返した形だけを見て、差し戻しの理由にする。
| 形 | 扱い |
|---|---|
| 期間ラベルの文字列列（`時点`）で並べ替える（`2025年9月` が `2025年12月` より後に来る） | **硬い問題**。parse-period → `periodStart` で並べ直させる。直らなければ適用しない |
| 設計時プレビューが 0 行（画面のプレビューが空になる） | **硬い問題**。どの条件が空振りしたか（実在する値の例つき）を添えて見本の値を直させる |
| 粒度が混在する期間列を parse-period で開いたのに、粒度で絞る filter が無い（月次と年次が混ざる） | **柔らかい問題**。1 回は差し戻すが、直らなければ（「全行を」のように意図的なこともある）警告つきで適用し、返答の下に黄色の注記で出す |

このほか、12B の機械的な書き間違いは差し戻す前に決定的に直す: ノード id の `_`/大文字の揺れ、結合キーを `["時点","時点"]` の配列で書く、引数に束縛した filter の `value` を空にする（実在値を種にする。日付は期間の範囲の最小・最大）、`valueBinding.source` の綴り違い、`agent-output` の必須項目の書き忘れ。実測ではこれらが差し戻しの理由の大半だった。

**文脈の消費と履歴の管理**: 見出しの下に「文脈 8.6k / 200k（4%）」のメーターが出る（直前の応答でモデルが数えたトークン数と、LM Studio から取れる読み込み中モデルの文脈の長さ。取れなければ「約 8.6k トークン」）。70% で黄、90% で赤になり、赤のときは圧縮かクリアを促す。「**履歴を圧縮**」は会話が 5 ターン以上のとき押せ、直近 4 ターンを残して古いターンをモデルに要約させる（`POST /tool-drafts/design-chat/compact`）。要約は「圧縮済み（N ターン）」として先頭に畳んで置かれ、次のターンからは要約が送られる。**圧縮したターンの変更は取り消せなくなる**。「**履歴をクリア**」は会話・要約・メーターを消す（キャンバスは変えない）。消費の大半はグラフとノードカタログで、会話は小さい（実測: 6 ターンで約 8.7k トークンのうち会話は 1k 前後）ので、圧縮で減るのはその分だけである。

**Tool Calling 契約の説明の更新**: 操作の語彙に `set-agent-tool` があり、エージェント向けの説明文（「エージェント向けコンテキスト」の説明）を書き換えられる。「説明文に引数の書式と粒度の綴りを書いて」のような指示で更新できる。プロンプトの規則は「引数を足した・変えたときは頼まなくても同じ応答で説明文を更新する」とモデルへ指示しており、実測（地域の引数を足した応答で「target_region を指定すると…省略すると全地域」が説明に入った例）でも効いた。ただしこれはLLMへの指示であって強制ではないため、**毎回確実に守られる保証はない**（更新し忘れる・頼んでいない変更が混ざる応答もありうる）。実際に何を変えたかは返答の下の変更一覧で確認する。取り消しは説明文も戻す。

**取り消し**: 各アシスタントの返答に「この変更を取り消す」ボタンが付く。押すとそのターンの適用前のグラフへ戻る。それより後のターンで加えた変更もまとめて戻り、会話自体は消えない（応答に「取り消し済み」と表示される）。

**モデル未設定のとき**: ボタンは押せるがパネルの入力欄が無効になり、関数電卓の「AIに式を書かせる」と同じ文言（「ローカルLLMが未設定です。設定 > モデル で main スロットを設定して再読み込みすると、AIに式を書かせられます。」）で設定箇所を案内する（隠さない）。有効化条件は分析設定補助・関数電卓アシスタントと同じ（main スロットにモデルが設定済みで、構造化出力に対応していること）。

**実測の目安**（LM Studio、12B級、e-Stat の総人口・給与・労働時間データ。8 パターン = 作成中の修正 3 ターン / 壊れたツールの修復 / 2 ソースの結合＋計算列 / 日付引数 / 曖昧な指示 / 未登録データ / 集計 / ノードの削除）: 8 パターンすべてが意図どおりに適用され、曖昧な指示は聞き返し、未登録データはデータソース画面への登録を案内した。1 ターン 5〜180 秒（差し戻しが入ると 2 倍）。以下は最初の 3 ターンの例。 空のキャンバスから「都道府県別の総人口を年次に絞って多い順に10件」→「地域名を引数で絞れるように。省略したら全地域」→「出力が溢れる心配は？」の3ターンが差し戻しなしで通り、プレビューも10行返った。所要時間は1ターン目が約65秒、2ターン目が約45秒、3ターン目（質問のみ）が約10秒。実測で見つかった機械的な書き間違い（ノードIDの `_` や大文字の揺れ、引数に束縛したフィルタの値を空にする、`agent-output` の行数・バイト数・溢れ方の設定を書き忘れる、の3種）は、差し戻しに頼らず決定的な正規化で吸収している。

API は [docs/04 §3.9](./04-api-spec.md#39-設計アシスタント) の `POST /tool-drafts/design-chat`。実装は `src/application/tool/design-tool-chat.ts`（段取り）・`design-rules.ts`（意味の検査。Factory と共有）・`design-chat-agent-tool.ts`（説明文の更新）・`design-chat-compact.ts`（履歴の圧縮）、`src/domain/etl/graph-edit.ts`、`src/api/draft-tool-routes.ts`、`src/ui/tool-builder/DesignChatPanel.tsx`。設計判断は [ADR-0051](./adr/0051-tool-design-chat.md)、実装契約は [implementation/v47](../implementation/v47-tool-design-chat.md)。

### 3.18 ノードの自動配置（折り返し階層レイアウト）

テンプレートや設計アシスタントで作ったツールは 10 ノード前後の一本道になりやすく、左から右へ 1 列に並べると 12 ノードで横幅が約 2,700px になり、全体を画面に収めると文字が読めなかった。そこで、**上流からの段数（最長経路の深さ）で列を決め、決めた列数を超えたら次の帯へ折り返す**配置にした（`src/ui/tool-builder/layout.ts` の `layoutWrapped`）。

- **列と帯**: 入力の無いノードが 0 段目。既定は 4 列（最小 3 列）で、5 段目以降は下の帯の左端から続ける。同じ段のノードは上流の並び順の平均（重心）で上下を並べ、辺の交差を減らす。
- **どこにもつながらないノード**（`agent-input`・`current-datetime` など、フィルタの値の束縛だけで使われるもの）は、最上段の「引数の帯」に並べる。
- **自動で配置する場面**: テンプレートから作成したとき、保存済みツールを開いてどのノードにも位置が無いとき、空のキャンバスに設計アシスタントがノードを置いたとき。手で動かした位置があるツールは動かさない。設計アシスタントが既存のキャンバスへノードを足すときは上流の隣に置き、右端を越えるなら次の帯へ折り返す。
- **「整列」ボタン**: キャンバス左上。押すといまの表示幅に収まる列数（ノード幅と間隔から計算、最小 3 列）で全ノードを並べ直し、全体が見えるよう表示を合わせる。直後に「整列しました / 元に戻す」が出て、1 回だけ整列前の位置へ戻せる。
- 実測: e-Stat の給与と労働時間を結合して時間当たり給与を出すテンプレートのツール（12 ノード）が、横 1 列から 4 列×3 帯＋引数の帯になり、幅 784px のキャンバスにノード名が読める大きさで収まった。

実装契約は [implementation/v51](../implementation/v51-auto-layout.md)。
...
| 制限（実行） | 1 ノードの生成行数の上限は既定 250,000 行（`DEFAULT_MAX_EXECUTION_ROWS`。運用者が環境変数 `AGENTCONTEXT_MAX_EXECUTION_ROWS` で変えられる）。超過は切り捨てず `nodeId` 付きの `ETL_SCHEMA`（422）で止め、エラー文が「上流で行を絞るか、この環境変数を上げる」と言う。`join` は出力が既定 100,000 行で止まる（`DEFAULT_JOIN_MAX_ROWS`）。この上限は**結合ノードの設定「最大行数」（`maxRows`、1〜10,000,000）で結合ごとに変えられる**。キーの指定ミスによる直積を「結合キーを確認して」という具体的な文で早く止めるための安全弁なので、キーが正しいのに止まるときだけ上げる。実際に効くのは結合の上限とサーバーの上限の小さい方。`time-series-analysis` の欠損 bucket 補完はパーティションあたり 100,000 bucket（`MAX_TIME_SERIES_FILL_BUCKETS`）で先に止まる。Agent 実行の Tool call も同じ規則で全行を計算する（[07 §3](./07-execution-model.md#3-agentチャット実行tool-calling)） |
...
- undo / redo・複製・コピー（キャンバスの基本作法）— **未実装**。ノードの移動・追加・削除・設定変更を汎用に戻す／やり直す・ノードを複製する・コピー&ペーストする機能はどれも無い（`src/ui/tool-builder/store.ts` に該当ロジックが無いことを確認済み）。似た名前の機能と混同しないこと: §3.18 の「整列」ボタンは整列した**直後だけ**1 回、整列前の位置へ戻せる（`undoArrange`）。§3.17 の設計アシスタントは各応答に付く「この変更を取り消す」で、そのターン以降にアシスタントが加えた編集をまとめて戻せる。どちらもキャンバス操作全般の undo / redo ではない
- 自動整列 — §3.18 の折り返し階層レイアウトと「整列」ボタン（1 回だけ元に戻せる）で実装済み
