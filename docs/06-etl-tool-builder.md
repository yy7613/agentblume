...
6. **作成** — サーバーが実体化し、**手で組んだ Tool と同じ検査**（構造・列の存在・設計時プレビュー）を通してからグラフを返す。画面はそれをキャンバスへ展開し、表示名・公開名・Agent Tool 契約（名前と説明）を埋める。保存はしない（所有者を入れて明示的に保存する）。
...
API は [docs/04 §3.9](./04-api-spec.md#39-設計アシスタント) の `POST /tool-drafts/design-chat`。実装は `src/application/tool/design-tool-chat.ts`、`src/domain/etl/graph-edit.ts`、`src/api/draft-tool-routes.ts`、`src/ui/tool-builder/DesignChatPanel.tsx`。設計判断は [ADR-0051](./adr/0051-tool-design-chat.md)、実装契約は [implementation/v47](../implementation/v47-tool-design-chat.md)。
