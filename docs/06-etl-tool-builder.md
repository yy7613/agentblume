...
4. **作成** — サーバーが実体化し、**手で組んだ Tool と同じ検査**（構造・列の存在・設計時プレビュー）を通してからグラフを返す。画面はそれをキャンバスへ展開し、表示名・公開名・Agent Tool 契約（名前と説明）を埋める。保存はしない（所有者を入れて明示的に保存する）。
5. 展開後、どのテンプレートから作ったか（`id@version`）を通知として出す。
