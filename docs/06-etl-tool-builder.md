...
5. 返答には変更の一覧（「引数 region (string, 省略可) を追加」のような要約）が並ぶ。質問への回答や聞き返しなど、キャンバスを変えない返答は変更一覧が空のまま返る。
6. 直近12ターンまでの会話を次の指示と一緒に送るので、「続けて地域を引数で絞れるように」のように前のターンを踏まえた指示が書ける。別のツールを開く・新規作成・テンプレートから作成すると会話は消える。

**操作の語彙（モデルが書けるのはこの5つだけ）**
...
**Tool Calling 契約の説明の更新**: 操作の語彙に `set-agent-tool` があり、エージェント向けの説明文（「エージェント向けコンテキスト」の説明）を書き換えられる。「説明文に引数の書式と粒度の綴りを書いて」のような指示で更新でき、**引数を足した・変えたときは頼まなくても同じ応答で説明文が更新される**（実測: 地域の引数を足した応答で「target_region を指定すると…省略すると全地域」が説明に入った）。取り消しは説明文も戻す。
