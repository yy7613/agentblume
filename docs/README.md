...
| **ツールテンプレート** | ツール一覧の「テンプレートから作成」 | Toolの構成（ノード・引数・スロット）をあらかじめ用意した外部ファイル。データソースと列を選ぶだけでToolのキャンバスができあがる。標準で10本同梱し、利用者も追加できる。左ナビの「業務テンプレート」（下記）とは別物 |
| **業務テンプレート** | 左ナビ「作る → 業務テンプレート」 | 仕訳・経費精算・請求消込・契約レビューという**業務そのもの**の画面一式（実験的な機能）。Toolを作るための「ツールテンプレート」とは別の機能で、名前だけが似ている |
...
| **設計アシスタント** | ツール作成画面のヘッダーのボタン | 自由な文章で指示するとキャンバスのノードが増えたり変わったりするチャットパネル。保存はしない。各ターンから1手で取り消せる |
...
| **スロット** | ツールテンプレートの可変部分（データソース・列・結合キー・数値・自由記述など）。型ごとの候補から選ぶと、実体化のときにノードやconfigへ差し込まれる |
| **検証の前提（scenario grounding）** | Agent Factoryが生成するScenarioの `context` へ決定的に付け足すブロック。対象Agentが持つデータの範囲（期間・カテゴリ・値の列）を擬似ユーザーへ伝え、データに無い数値を自作して渡すような噛み合わない検証を防ぐ |
...
| **プロンプトファイル** | 実装がモデルへ送る指示文（system の規則・差し戻しの文・タスクの目的）を持つ外部ファイル（`prompts/**/*.md`）。ツールテンプレートと同じ「同梱 + 環境変数で足す置き場」の二層構成だが、こちらは節が1つ欠けても起動時に止まる。人が編集して保存する成果物（Agent の system プロンプト・Skill の文など）とは別物 |
