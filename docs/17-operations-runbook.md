...
| `auditDays` | 365日 | 監査ログ（誰が何をしたか）を削除する |

> 監査ログの既定が trace（14日）よりずっと長いのは意図的である。監査が答える問いは「先月あの設定を変えたのは誰か」で、実行トレースと同じ寿命では用を成さない。`auditDays` を短くして即適用すれば消せてしまうので、**保持期限の変更と適用そのものが監査対象**になっている。
