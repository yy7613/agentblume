...
| env | 既定 | 内容 |
|---|---|---|
| `AGENTCONTEXT_TOOL_TEMPLATES_DIR` | 未設定（同梱の `templates/tools` だけを読む） | ツールテンプレート（[ADR-0049](./adr/0049-tool-templates.md)）の追加の置き場所。`;` か `:` で区切って複数指定できる（Windowsのドライブ接頭辞 `C:\` の `:` は区切りにならない）。同じ `id` は後勝ち（この置き場が同梱の標準テンプレートを上書きできる）。ファイルはリクエスト時に読み、更新時刻とサイズでキャッシュするので再起動なしで反映される。**v48 で起動時の環境変数検証に加わった**（区切り記号だけの値のような「書いたのに1つも置き場所にならない」設定ミスは起動時に弾く） |
...
### プロンプト（モデルへ送る指示文・local）

| env | 既定 | 内容 |
|---|---|---|
| `AGENTCONTEXT_PROMPTS_DIR` | 未設定（同梱の `prompts/` だけを読む） | モデルへ送る指示文（[ADR-0052](./adr/0052-prompt-files.md)）の追加の置き場所。`;` か `:` で区切って複数指定できる（Windowsのドライブ接頭辞 `C:\` の `:` は区切りにならない）。同じ `id` は**ファイル単位**で後勝ち（この置き場が同梱の文を上書きできる）。ファイルの更新時刻・サイズを見て再読込するので再起動なしで反映される |

ツールテンプレートと違い、**壊れた・節を欠いた置き場所は起動を止める**。起動時にコードが宣言した id と節（`PromptSpec`）が全部揃っているかを検査し、欠けていれば「プロンプトファイルの検査に失敗しました」とファイル名・欠けた節・直し方を並べて起動を中止する。実行中の再読込で壊れたファイルは直前の良い版を使い続け、警告をログに残すだけで止めない（編集途中の保存でサーバーが落ちないように）。書き方は[`prompts/README.md`](../prompts/README.md)、詳細は[implementation/v48](../implementation/v48-prompt-files.md)。
...
| env | 既定 | 内容 |
|---|---|---|
| `AGENTCONTEXT_MAX_EXECUTION_ROWS` | `250000` | ETLの1ノードが生成してよい行数の上限（1以上の整数）。キーの指定ミスによる直積などでメモリが尽きるのを止める安全弁で、サーバー全体（プレビュー・保存済みToolの実行・Factory）に効く。結合ノードの「最大行数」をこれより大きくしても、この上限で止まる。上げるとその分メモリを使う |
