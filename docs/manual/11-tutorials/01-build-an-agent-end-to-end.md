...
3. キャンバスに見本の 2 ノード（「JSON source」と「行フィルター」）がある。1 つずつクリックして `Delete` キーで消し、キャンバスを空にする。
...
- キャンバスに「CSV source」「Parse period」「Sort」「Limit」「Agent output」の 5 ノードがつながって置かれた。
- 返答の変更一覧に `added csv-source …`、`added parse-period …`、`added sort …`、`added limit …`、`added agent-output …` の 5 行が並んだ。
- 画面上部の表示が「有効な草案」になった。
...
確かめること: 変更一覧に `added agent-input …`（引数の宣言）と `added filter …`（行フィルター）の**両方**があるか。

実際に通したときは、返答の文は「フィルタを追加しました」なのに、変更一覧には `added agent-input 'input-region' …` しか無かった。このように**足りないときは、ノード名を挙げて頼み直す**。

```text
変更一覧に行フィルターが無い。region 列を引数 region で絞る行フィルターを src-sales の直後に足して
```

返答の例（43 秒）: 「src-sales の直後に、引数 region を使用した行フィルタを追加しました。」 変更一覧に `added filter 'filter-region' after 'src-sales', before 'parse-month' …` が出た。
...
2. 最後のノード（`out-results`）をクリックし、画面下の「プレビュー」を見る。
...
3. 画面下の「エージェント向けコンテキスト」で、「ツール名」に `monthly_sales_by_region` を入れる。**「ツール名」が空のまま保存すると、2-6 で書かせた説明文は保存されない**（実際に通したときにそうなった）。
...
| 月 | 売上 | 単価 |
|---|---|---|
| 2026年7月 | 110,000 | 297.3 |
| 2026年6月 | 101,000 | 280.6 |
| 2026年5月 | 98,000 | 280 |

![チャットの返答と、ツール呼び出しの記録](../assets/11-tutorials/chat-answer.png)

確かめること: 返答の下の記録に `ツール呼び出し monthly_sales_by_region({"region":"West"})` と、ノードごとの行数（`src-sales:6, … filter-region:3, … out-results:3`）が出ている。エージェントがツールに正しい引数を渡し、ツールが West の 3 行を返したことがわかる。数字はデータどおり（110000 ÷ 370 = 297.3）である。
...
| ノードが画面の外にある | 既存のキャンバスに足したノードは上流の隣に置かれる | キャンバス左上の「整列」 |
| 「バージョンを保存」が押せない | 表示名・メタデータが空、またはグラフにエラー | ボタンの下の理由を読んで埋める・直す |
| 保存したツールを開き直したら説明文が消えていた | 「ツール名」が空のまま保存した | 「エージェント向けコンテキスト」の「ツール名」と「説明」を入れて保存し直す |
