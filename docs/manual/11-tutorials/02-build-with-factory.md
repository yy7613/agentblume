...
枠の上の文（`Review the proposed plan for …`）は英語で出る。
...
| 候補 | `22cfee24-…@1.0.0` | 使うことを勧めるエージェントの版（ID で出る） |
...
表の上の 1 行の総括（`Factory run stopped after 1 iteration(s); …`）は英語で出る。
...
2. キャンバスに「CSV source」「Parse period」、3 つの「Filter」（粒度・地域・期間）、「Sort」「Limit」「Agent output」と、引数の「Agent input」が並んでいた。
