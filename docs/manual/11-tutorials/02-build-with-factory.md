...
| 候補 | `販売分析アシスタント (Factory)@1.0.0` | 使うことを勧めるエージェントの版。表示名で出る。画面を開いたまま完了を待った場合も、表示名が引けなければエージェントの一覧を読み直して表示名にする（v54）。表示名が空のときだけ内部ID（`22cfee24-…@1.0.0` の形）で出る |
...
表の上の 1 行の総括は日本語で出る（v53。以前は `Factory run stopped after 1 iteration(s); …` のような英語だった）。例えば「1 回のイテレーション後にFactory runが停止しました（直近の目標達成率=1.00、平均満足度=5.00）。」のように出る。改善を回した回の総括（モデルが書く文）は、実行を始めたときの表示言語で書かせる。品質判定の下の理由（例: 「平均満足度 3.00 が目標値 4 を下回っています。」）も日本語で出る（v54）。
