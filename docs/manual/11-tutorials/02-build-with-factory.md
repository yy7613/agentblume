...
計画ができると、状態が「承認待ち」になり、「計画の承認が必要です」の枠が出る。実測では開始から約 2 分半で出た。

![計画の承認を求められたところ](../assets/11-tutorials/factory-plan.png)

> 画像は v53 以前の表示。枠の上の文（`Review the proposed plan for …`）は、現在は下の実測例のとおり日本語で出る。

枠には、作るエージェントの名前と説明、作るものの数が出る。実測の計画は次のとおりだった（例）。

- エージェント: 売上分析アシスタント — 営業部のマネージャー向け、SQL不要の売上・販売数データ集計および推移レポート生成アシスタント。月次データに基づき、地域別の売上や数量の動向を瞬時に回答します。
- 「ツール: 1」「スキル: 1」「ペルソナ: 1」「シナリオ: 2」

枠の上の文は日本語で出る（v53。以前は `Review the proposed plan for …` のような英語だった）。例えば「「月次の売上データについて、地域別・月別の売上や販売数の質問に答え、推移を要約できるアシスタントが欲しい」の計画案です: エージェント「売上分析アシスタント」、Tool 1 件、Skill 1 件、ペルソナ 1 件、シナリオ 2 件。」のように出る（やりたいことの全文がそのまま入るので長くなる）。
...
> 画像は v53 以前の表示。表の上の総括の 1 行（`Factory run stopped after …`）と「候補」の行（内部IDのみ）は、現在は下の実測例のとおり日本語の総括・表示名（引ければ）で出る。
...
| 候補 | `売上分析アシスタント (Factory)@1.0.0` | 使うことを勧めるエージェントの版。表示名が引けるときは表示名で出る（v53。それ以前・表示名が空のときは `22cfee24-…@1.0.0` のような内部IDで出る） |
...
| 生成資産（draft） | ツール: 1、スキル: 1、エージェント版: 1、ペルソナ: 1、シナリオ: 2 | できたものの数。すべて draft（下書き）で保存される |

表の上の 1 行の総括は日本語で出る（v53。以前は `Factory run stopped after 1 iteration(s); …` のような英語だった）。例えば「1 回のイテレーション後にFactory runが停止しました（直近の目標達成率=1.00、平均満足度=5.00）。」のように出る。
