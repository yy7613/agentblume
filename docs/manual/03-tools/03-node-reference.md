...
この節は辞書として引く。ツール画面の左「ノード」に並ぶ全ノードについて、何ができるか・主な設定・よくある使い方・注意をまとめる。名前はパレットの日本語表記（キャンバスのカードには英語の型名が出るので、括弧内に併記する）。

ノードは 4 分類ある。

| 分類 | カードの表示 | 役割 |
|---|---|---|
| 入力 | `SOURCE` | データを読む・引数を宣言する。入口の丸は無い |
| 変換 | `TRANSFORM` | 行を絞る・列を変える・つなぐ・並べる |
| 分析 | `ANALYZE` | 集計・統計を計算する。結果は普通の表として次のノードへ渡る |
| 出力 | `OUTPUT` | 結果の渡し方を決める。**1 つのツールに 1 つだけ、最後に置く** |
