...
キャンバス上のノードのカードには、種類（`SOURCE` / `TRANSFORM` / `ANALYZE` / `OUTPUT`）、ノードの型名（英語表記。例 `CSV source`・`Filter`）、ノードID（例 `csv-source-1`）、出力の列がどこまで確定しているかのバッジ（「確定」「推論」「部分」「不明」「不一致」）が出る。問題があればカードの下に赤い文、つながっていなければ「未接続」が出る。
...
2. 左の「入力」から「**CSV入力**」を押す。
   → キャンバスに `CSV source` が置かれ、右のインスペクターに「CSV入力」の説明が出る。
...
1. `CSV source` を選んだまま、「変換」の「**行フィルター**」を押す。
   → `Filter` が右に置かれ、線でつながる。インスペクターに条件の欄が出る。
2. インスペクターの「列」に `region`、「演算子」は `=`、「値」に `East` を入れる。
   → プレビューが East の 3 行になる。
3. `Filter` を選んだまま「**並べ替え**」を押し、「設定を開く」→「**キーを追加**」→「対象列」で `month`、「順序」で `desc` を選び、「設定を適用」を押す。
   → 新しい月から順に並ぶ。
4. `Sort` を選んだまま、「出力」の「**エージェント出力**」を押す。
   → 終端に `Agent output` がつながる。インスペクターに「直接返却 · rows · 100行」と出る。
...
1. キャンバスの何もない所をクリックして選択を外し、「入力」の「**エージェント入力**」を押す。
   → `Agent input` が置かれる。これは**どこにもつながない**。引数の宣言として置いておくノードである。
2. 下右の「Tool Calling契約」の「引数」で、最初の行の名前を `region` に変え、「必須」のチェックを外す（省略されたら全地域を返したいので）。
3. `Agent input` を選び、インスペクターの「詳細テキスト編集」を開いて、「サンプル引数」を `{"region": "East"}` にする（プレビューで使う見本の値）。
4. `Filter` をクリックして選び、インスペクターの「**条件値の取得元**」を「**エージェント入力**」に変え、「**エージェント入力フィールド**」で `region` を選ぶ。
