...
> 画像は v53 以前の表示。ノードのカードの区分（`SOURCE`）と型名（`JSON source`）は、現在は「入力」「JSON入力」のようにパレットと同じ日本語で出る。

「新規作成」で開いたキャンバスには、見本として「JSON入力」と「行フィルター」の 2 つのノードが置いてある。アシスタントはいまのキャンバスに**足したり直したり**するので、空から作るときは、先にこの 2 つを選んで `Delete` キーで消しておく（本節の例はそうしている）。
...
このとき、「CSV入力」→「期間の解釈」→「並べ替え」→「行数制限」→「エージェント出力」の 5 ノードが置かれた（キャンバスのノードのカードにも同じ日本語名と、`src-sales` のようなノード ID が出る）。`2026-05` のような期間の文字列はそのままでは正しく並ばないので、アシスタントは「期間の解釈」ノードで開始日の列 `periodStart` を作り、それで並べている。空のキャンバスに置いたノードは自動で折り返して並ぶ。
...
> 画像は v53 以前の表示。変更一覧（`added calculate '…' after '…'` 等）は、現在は本文のとおり日本語（「関数電卓「…」を追加しました（「…」の後ろ）」等）で出る。

1 回の返答は、上から次の順に並ぶ。

1. **自分の指示**（色付きの吹き出し）
2. **アシスタントの説明**（指示と同じ言語）
3. **変更一覧**（箇条書き）— 実際にキャンバスへ当てた変更の記録。日本語で出る（v53）。たとえば「**行フィルター「filter-region」を追加しました（「src-sales」の後ろ）**」（ノード `filter-region` を `src-sales` の後ろに追加した）、「**行数制限「lim-all」を削除しました**」（削除した）、「**並べ替え「sort-m」の設定を変更しました: …**」（設定を変えた）、「**「region」を行フィルター「filter-region」につなぎました**」（つないだ）、「**エージェント向けのツール説明を設定しました（…）**」（説明文を書き換えた）のように、種別名（`filter` → 「行フィルター」など）もパレットと同じ日本語で出る。
4. **赤い枠**（出たときだけ）— 先頭に「**適用していません（キャンバスは変わっていません）。上の返答の変更は、次の理由で通りませんでした:**」と出て、続けて理由が並ぶ（v53。アシスタントの説明文が「追加しました」のように言っていても、この枠が出たターンは**キャンバスを変えていない**）。理由を読んで指示を直す。
...
> 画像は v53 以前の表示。変更一覧（`removed limit '…', connecting '…' to '…'` 等）は、現在は本文のとおり日本語（「行数制限「…」を削除しました（「…」と「…」をつなぎ直しました）」等）で出る。
...
- 引数や列を足した返答で、頼まなくても説明文が書き換わることがある（実測: 日付の引数 `from` と計算列 `unit_price` を足したとき）。書き換わらないこともある（実測: 引数 `region` を足したとき）。変更一覧に「エージェント向けのツール説明を設定しました」が無ければ、上のように頼む。
- 「この変更を取り消す」は説明文も戻す。
- **「ツール名」が空のまま保存しても、アシスタントが書いた説明文はちゃんと保存される**（v53で直った。それまでは説明文だけ書いて「ツール名」が空だと、保存時に説明文が黙って消えていた）。「ツール名」が空なら、保存時に代わりに「公開名」が使われる。「ツール名」だけ入れて「説明」が空のときは、既定の説明（`表示名 (副作用)`）で補われる。両方空なら、以前と同じく何も送られない。
- 補った名前（公開名）がモデルへ公開できない形（日本語・空白・記号を含む）だと、保存ボタンの下に「「…」はモデルへ公開できる function 名ではありません。…」と出て保存が止まる。公開名か「ツール名」を英数字・`_`・`-` の 1〜64 文字に直す。
...
2. キャンバスが見づらければ、キャンバス左上の「整列」を押す（直後の「元に戻す」で 1 回だけ戻せる）。アシスタントが既存のキャンバスに足したノードが表示範囲の外になったときは、表示が自動で合う（v53。全体を映しても見えなければ、足したノードだけへ寄る）。
3. 表示名と「メタデータ」（内部ID・作業名・公開名）を入れる。「エージェント向けコンテキスト」の「ツール名」は空でも保存できるが、入れておくとエージェントに見える名前が分かりやすくなる。
...
「設計アシスタント」のボタンは押せるが、欄の入力欄と「送信」が使えず、「設計アシスタントを使うにはモデルが必要です。設定画面の「モデルプロバイダ」でメインモデル（main スロット）を設定してから、このパネルを開き直すと、会話でツールを設計できます。」と、下に「**設定画面を開く**」ボタンが出る（v53。以前は関数電卓と同じ案内文で、直す場所への導線も無かった）。

1. 「設定画面を開く」を押す（または左ナビの「設定」を開く）。「モデルプロバイダ」の「メインモデル」を設定して「保存」する（[モデルの設定](../01-getting-started/02-model-settings.md)）。
...
| ノードが画面の外にある・重なって見えない | 既存のキャンバスに足したノードは上流の隣に置かれる。通常は自動で表示が合う（v53） | それでも見えないときはキャンバス左上の「整列」を押す |
