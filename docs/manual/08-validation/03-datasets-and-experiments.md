...
この節を読むと、評価に使う事例を集めてデータセットにし、採点の仕方（評価プロファイル・審査ルーブリック）を決め、実験を回してスコアを読めるようになる。
...
- 「**コード採点**」… 決まったロジックで採点する。「**スコアラー**」から選ぶ（`keyword-coverage`・`completeness`・`tone-consistency`・`content-similarity`）。
- 「**LLM判定**」… LLMに採点させる。保存済みの「**審査ルーブリック**」から選ぶ。
...
| 症状 | 原因 | 直し方 |
|---|---|---|
| 「実験を開始」の上に「判定モデルが未設定です」と出る | 評価プロファイルが審査ルーブリックを使うのに、judgeスロットにモデルが無い | 「設定で判定モデルを設定」から設定画面へ行き、judgeスロットにモデルを保存する |
