...
> **Agent Factory が作るシナリオの `context`** には、計画の状況設定の後ろに「検証の前提」ブロック（見出し `# Validation premises / 検証の前提 (factory-managed)`）が決定的に付く。対象エージェントが持つデータ（名前・値の列・期間の範囲・カテゴリの例）と、「自分で数値を作って渡さない」「範囲内で質問する」という規律を擬似ユーザーへ伝えるためである（[docs/16 §4 Stage 5](./16-agent-factory.md) / [ADR-0050](./adr/0050-scenario-grounding.md)）。手で作るシナリオには付かないので、対象エージェントの守備範囲は `context` に自分で書く。

> **アンケートの評点の向き**: scale 設問は、モデルへ渡すスキーマの説明に `(integer 1..5; 1 = lowest / 最低, 5 = highest / 最高)` を付け、指示文でも「数が大きいほど高評価。自由記述の内容と評点を一致させる」と伝える。向きを書かないと、ローカル 12B は自由記述で絶賛しながら評点 1 を付ける（1 を「1 位」と読む。実測）。既定の設問 `q4` は「手間の少なさ（手間が少なかったほど高い点）」。
