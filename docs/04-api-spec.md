...
// POST /tool-drafts/design-chat
{
  ...
  "transcript": [                                    // 任意。直近の会話（画面が持つ。古い順、最大40ターン。
                                                       // モデルへ送る直近12件（発言=メッセージ単位。ユーザー/アシスタント
                                                       // それぞれ1件）への切り詰めは応用層が行う）
...
// → 200
{
  ...
  "promptTemplateVersion": "design-chat/v3"
}
```
...
- **所有者（`owner`）は省略可**: 共通メタデータを保存する要求（Tool・Skill・Agent・マルチエージェント・検証資産 等）の `owner` は省略・空文字・空白のみでよく、そのときはサーバーがログイン中の主体の表示名（無ければ `subject`。単一利用者モードでは `Local operator`）で埋める。明示した値はそのまま保存する。`owner` は表示用のラベルで、認可には使わない（「自作のみ」の判定は認証主体。[08](./08-security-auth.md)）。