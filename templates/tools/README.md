...
| 引数の必須 / 任意を作成画面で切り替えさせない | 引数に `"lock": { "ja": "…のため、必須に固定しています", "en": "Required: …" }`（理由は画面にそのまま出る） |
...
- 引数の `nullable` は既定値で、作成画面で引数ごとに切り替えられる。任意の引数は省略されると束縛した条件ごと外れるので、**外れると結果の意味が変わる引数**（粒度など）には `lock` を書いて切り替えさせない。任意 → 必須に切り替えられるのは `sample` を書いた引数だけ。
